...
extension SpacingExtensions on BuildContext {
  ...
  Widget get gapXS => SizedBox(width: HSizes.spacingXS, height: HSizes.spacingXS);

  /// Small gap (8px) - For compact layouts, button spacing
  Widget get gapSM => SizedBox(width: HSizes.spacingSM, height: HSizes.spacingSM);

  /// Medium gap (16px) - For standard content spacing, form fields
  Widget get gapMD => SizedBox(width: HSizes.spacingMD, height: HSizes.spacingMD);

  /// Large gap (24px) - For section spacing, card separation
  Widget get gapLG => SizedBox(width: HSizes.spacingLG, height: HSizes.spacingLG);

  /// Extra large gap (32px) - For major section breaks
  Widget get gapXL => SizedBox(width: HSizes.spacingXL, height: HSizes.spacingXL);

  /// Double extra large gap (48px) - For dramatic spacing, screen sections
  Widget get gapXXL => SizedBox(width: HSizes.spacingXXL, height: HSizes.spacingXXL);
}
...
extension PaddingExtensions on BuildContext {
  ...
  EdgeInsets get paddingXS => EdgeInsets.all(HSizes.spacingXS);

  /// Small padding (8px all sides) - For compact elements, buttons
  EdgeInsets get paddingSM => EdgeInsets.all(HSizes.spacingSM);

  /// Medium padding (16px all sides) - For standard content, cards
  EdgeInsets get paddingMD => EdgeInsets.all(HSizes.spacingMD);

  /// Large padding (24px all sides) - For spacious layouts, dialogs
  EdgeInsets get paddingLG => EdgeInsets.all(HSizes.spacingLG);

  /// Extra large padding (32px all sides) - For major spacing, screen edges
  EdgeInsets get paddingXL => EdgeInsets.all(HSizes.spacingXL);

  /// Double extra large padding (48px all sides) - For dramatic spacing
  EdgeInsets get paddingXXL => EdgeInsets.all(HSizes.spacingXXL);
  ...
  EdgeInsets get paddingResponsive => HSizes.getResponsivePadding(mediaQuery.size.width);
}

/// Border radius extensions
extension BorderRadiusExtensions on BuildContext {
  /// Extra small border radius (4px) - For subtle rounding, checkboxes
  BorderRadius get radiusXS => BorderRadius.circular(HSizes.radiusXS);

  /// Small border radius (8px) - For buttons, chips, form fields
  BorderRadius get radiusSM => BorderRadius.circular(HSizes.radiusSM);

  /// Medium border radius (12px) - For cards, containers, most elements
  BorderRadius get radiusMD => BorderRadius.circular(HSizes.radiusMD);

  /// Large border radius (16px) - For prominent elements, dialogs
  BorderRadius get radiusLG => BorderRadius.circular(HSizes.radiusLG);

  /// Extra large border radius (24px) - For hero elements, bottom sheets
  BorderRadius get radiusXL => BorderRadius.circular(HSizes.radiusXL);

  /// Full border radius (circular) - For avatars, pills, circular buttons
  BorderRadius get radiusFull => BorderRadius.circular(HSizes.radiusFull);
...
extension ResponsiveExtensions on BuildContext {
  ...
  bool get isMobile => HSizes.isMobile(mediaQuery.size.width);

  /// Check if current device is tablet size (600px - 1200px width)
  bool get isTablet => HSizes.isTablet(mediaQuery.size.width);

  /// Check if current device is desktop size (>= 1200px width)
  bool get isDesktop => HSizes.isDesktop(mediaQuery.size.width);

  /// Get responsive text size multiplier
  double get textSizeMultiplier => HSizes.getTextSizeMultiplier(mediaQuery.size.width);
...
extension WidgetExtensions on Widget {
  ...
  /// Wrap with SafeArea
  Widget safeArea({bool top = true, bool bottom = true, bool left = true, bool right = true}) => SafeArea(top: top, bottom: bottom, left: left, right: right, child: this);
}
