...
class LaundryListView extends StatelessWidget {
  const LaundryListView({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(onPressed: controller.showAddLaundry, icon: const Icon(HIcons.addUser), label: const Text('Add Laundry')),
      body: SafeArea(
        child: Obx(() {
          // Show error state
          ...
          return Column(
            children: [
              ...
              // Laundry List
              Expanded(
                child: controller.filteredLaundryList.isEmpty
                    ? Center(
                        child: Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            ...
                            Text(
                              controller.selectedUser.value != null
                                  ? 'No laundry records found for ${controller.selectedUser.value}'
                                  : 'No laundry records found for ${controller.getMonthName(controller.selectedMonth.value)} ${controller.selectedYear.value}',
                            ),
                            const SizedBox(height: 8),
                            const Text('Tap the + button to add laundry'),
                          ],
                        ),
                      )
                    : ListView.separated(
                        ...
                        itemCount: controller.filteredLaundryList.length,
                        itemBuilder: (context, index) {
                          final laundry = controller.filteredLaundryList[index];
                          return Card(
                            elevation: HSizes.elevationLevel3,
                            color: context.colorScheme.primaryContainer,
                            child: ListTile(
                              leading: const Icon(Icons.local_laundry_service, size: HSizes.avatarSize),
                              title: Text(laundry.user, style: context.textTheme.titleLarge),
                              subtitle: Text(DateHelper.toReadableDate(laundry.date)),
                              trailing: Row(
                                mainAxisSize: MainAxisSize.min,
                                children: [
                                  Text('Qty: ${laundry.quantity}', style: context.textTheme.titleLarge),
                                  ...
                                  Obx(
                                    () => IconButton(
                                      ...
                                      onPressed: controller.isLoading.value ? null : () => _showDeleteConfirmation(context, controller, laundry),
                                    ),
                                  ),
                                ],
                              ),
  ...
  Widget _buildFilterSection(BuildContext context, LaundryController controller) {
    return Container(
      padding: const EdgeInsets.all(HSizes.spacingMD),
  ...
  Widget _buildUserCountsSection(BuildContext context, LaundryController controller) {
    return Obx(() {
  ...
  void _showDeleteConfirmation(BuildContext context, LaundryController controller, LaundryModel laundry) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Delete Laundry Record'),
          content: Text('Are you sure you want to delete the laundry record for ${laundry.user} on ${laundry.date}?'),
          actions: [
            TextButton(onPressed: () => Navigator.of(context).pop(), child: const Text('Cancel')),
            TextButton(
              onPressed: () {
                Navigator.of(context).pop();
                controller.deleteLaundry(laundry.id, laundry.user);
              },
